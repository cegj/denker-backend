...
export default class LikeController {
  ...
  static async like(req, res) {
    ...
    let denke = await Denke.retrieve({ id: denke_id });
    ...
    if (alreadyLiked[0]) {
      res.status(404).json({ message: "Você já curtiu este denke" })
      return
    }
    ...
    try {
      ...
      res.status(200).json({ message: "Denke curtido com sucesso", denke })
  ...
  static async unlike(req, res) {
    ...
    try {
      ...
      res.status(200).json({ message: "Denke descurtido com sucesso", denke })
